...
class particle:
    def __init__(self):
        ...
        self.loops = int(10000)                                                         # number of desired experiments
...
if __name__=="__main__":
    ...
    np.savetxt('collisionTimeData_'+str(p.N)+'particles_'+str(p.loops)+'experiments.txt', p.collisionTime)
...
'''
    ...
    # histograms
    binWidth = 1e-3
    nBins = int((np.max(p.collisionTime)-np.min(p.collisionTime))/binWidth)
    ...
    print('n',n)
    print('bins',bins)
    binAvg = np.ediff1d(bins)
    sqrTime = binAvg**2
    averageTime = np.dot(n,sqrTime)/np.dot(n,binAvg)
